...
const SingleNFTBidsTable: React.FC<ComponentProps> = ({ isLoggedIn = false, myBidsList, bidsList, onlyShowDownSm = false }): JSX.Element => {
        ...
        <Box display={(onlyShowDownSm) ? {xs: 'block', sm: 'block', md: 'none'} : {xs: 'none', sm: 'none', md: 'block'}} >
            <Stack direction="column" alignItems="left" justifyContent="space-between" marginTop={1}>
                ...
                {isLoggedIn && <>
                    ...
                    {myBidsList.length !== 0 && <Grid container spacing={1}>
                        <Grid item xs={6}>
